...
:: Ensure ParquetReader.exe exists in the dist folder
if not exist "%~dp0dist\ParquetReader.exe" (
    echo [ERROR] Compiled ParquetReader.exe was not found in the dist folder.
    pause
    exit /b
)
...
reg add "HKCU\Software\Classes\Applications\ParquetReader.exe\shell\open\command" /ve /t REG_SZ /d "\"%~dp0dist\ParquetReader.exe\" \"%%1\"" /f >nul
reg add "HKCU\Software\Classes\Applications\ParquetReader.exe\DefaultIcon" /ve /t REG_SZ /d "%~dp0dist\ParquetReader.exe,0" /f >nul
...
reg add "HKCU\Software\Classes\Antigravity.ParquetViewer\shell\open\command" /ve /t REG_SZ /d "\"%~dp0dist\ParquetReader.exe\" \"%%1\"" /f >nul
reg add "HKCU\Software\Classes\Antigravity.ParquetViewer\DefaultIcon" /ve /t REG_SZ /d "%~dp0dist\ParquetReader.exe,0" /f >nul
